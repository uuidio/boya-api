<?php
/**
 * @Filename ArticleRepository.php
 *
 * @Copyright       Copyright (c) 2015~2020 <http://www.shopem.cn> All rights reserved.
 * @License         Licensed <http://www.shopem.cn/licenses/>
 * @Author          moocde <mo@mocode.cn>
 */

namespace ShopEM\Repositories;

use ShopEM\Models\Article;

class ArticleRepository
{
    /*
     * 定义搜索过滤字段
     */
    protected $filterables = [
        // 'cat_id' => ['field' => 'cat_id', 'operator' => '='],
        'is_show' => ['field' => 'is_show', 'operator' => '='],
        'self_show' => ['field' => 'self_show', 'operator' => '='],
        'gm_id' => ['field' => 'gm_id', 'operator' => '='],
        'not_self' => ['field' => 'gm_id', 'operator' => '!='],
        'type' => ['field' => 'type', 'operator' => '='],//文章类型 0 文本 1 活动
        'verify_status' => ['field' => 'verify_status', 'operator' => '='],//审核状态 0 待审核 1 通过 2驳回

    ];
    /**
     * 文章列表字段
     *
     * @Author moocde <mo@mocode.cn>
     * @return array
     */
    public function aritcleListField($is_show='')
    {
        return [
            ['key' => 'id', 'dataIndex' => 'id', 'title' => 'ID'],
            ['key' => 'title', 'dataIndex' => 'title', 'title' => '标题'],
            ['key' => 'gm_name','dataIndex' => 'gm_name', 'title' => '所属项目','hide'=>isshow_models($is_show,['group'])],
            ['key' => 'type', 'dataIndex' => 'type', 'title' => '文章类型'],
            ['key' => 'verify_status', 'dataIndex' => 'verify_status', 'title' => '审核状态'],
            // ['key' => 'cat_name', 'dataIndex' => 'title', 'title' => '分类'],
            // ['key' => 'cat_id', 'dataIndex' => 'title', 'title' => '分类id', 'hide' => true],
            ['key' => 'listorder', 'dataIndex' => 'listorder', 'title' => '列表顺序'],
            ['key' => 'created_at', 'dataIndex' => 'created_at', 'title' => '发布时间'],

        ];
    }

    /**
     * 后台表格列表显示字段
     *
     * @Author djw
     * @return array
     */
    public function listShowFields($is_show='')
    {
        return listFieldToShow($this->listFields($is_show));
    }

    /**
     * 文章列表
     *
     * @Author moocde <mo@mocode.cn>
     * @return mixed
     */
    public function articleItems()
    {
        return Article::select(listFieldToSelect($this->aritcleListField()))->paginate(config('app.per_page'));
    }

    /**
     * 文章查询
     *
     * @Author moocde <mo@mocode.cn>
     * @param $request
     * @return mixed
     */
    public function search($request)
    {
        $model = new Article();
        $model = filterModel($model, $this->filterables, $request);
        $filter = listFieldToSelect($this->aritcleListField());
        // foreach ($filter as $key => $value) {
        //     if ($value == 'cat_name') {
        //         unset($filter[$key]);
        //     }
        // }
        $filter[] = 'gm_id';//追加属性需要但不展示在列表
        $filter[] = 'article_url';
        $filter[] = 'title_is_show';
        $filter[] = 'subtitle';
        $filter[] = 'activity_id';
        $filter[] = 'self_show';
        $lists = $model->select($filter)->orderBy('listorder', 'desc')->orderBy('id', 'desc')->paginate($request['per_page']);

        return $lists;
    }
}