<?php
/**
 * @Filename GroupGoodRepository.php
 *
...
//            ['dataIndex' => 'group_stock', 'title' => '拼团库存'],
     ...
     * @return mixed
     */
    public function groupBySearch($request)
    {
        $model = new Group();

        $model = filterModel($model, $this->filterables, $request);

        $model = $model->select('groups.*', 'groups.group_price as old_group_price', DB::raw('MIN(group_price) as group_price'));

        $lists = $model->orderBy('id', 'desc')->groupBy('goods_id')->paginate($request['per_page']);
