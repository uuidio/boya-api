...
class LiveController extends BaseController
{
    ...
    {
        $admin = notice::find(intval($request->id));
        if (empty($admin)) {
    ...
    public function noticeGet(Request $request)
    {
    ...
    {
        $repository = new LiveUsersRepository();
        $lists = $repository->getPlatformsShops();
     ...
     * @Author linzhe
     */
    public function bindingPlatform(Request $request)
    {
        $data = $request->only('platform_id', 'shop_id', 'id');

        if (!$data['platform_id'] || !$data['shop_id'] || !$data['id']) {
            return $this->resFailed(700, '请选择绑定对象');
        }

        $platform = DB::table('gm_platforms')->where('gm_id', $data['platform_id'])->first();
        if (!$platform) {
            return $this->resFailed(700, '品牌不存在');
        }

        $shop = DB::table('shops')->where(['gm_id' => $platform->gm_id, 'id' => $data['shop_id']])->first();
        if (!$shop) {
            return $this->resFailed(700, '品牌与门店不匹配');
        }


        LiveUsers::where('id', $data['id'])->update(['platform_id' => $data['platform_id'], 'shop_id' => $data['shop_id']]);
