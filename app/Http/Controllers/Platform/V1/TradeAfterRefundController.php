...
//        $postdata = input::get('data');
        ...
        $check_times = $service->isActionAllowed($refundsAct['refunds_id'], "do_refund", 2 * 1000, 1);
        if (!$check_times) {
            return $this->resSuccess([], '请勿频繁操作!');
        }

        $act_rewards=$refundsAct['refund_rewards']??1;
...
//                $res = 'payment.trade.refundpay' $apiParams;
                    ...
                    throw new \logicexception('其他方式不支持原路退款');
                }
            }
            ...
            if($act_rewards==1){
                //查看是否分销
                $tradeEstimates = TradeEstimates::where(['tid' => $refunds['tid'], 'status' => 0])->first();
                if (!empty($tradeEstimates)) {
                    //售后类型,1是取消退款,0是售后退款
                    if ($refunds['refunds_type'] == 1) {
                        $update_dates['status'] = 1;
                    } else {
                        $update_dates['status'] = 2;
                    }

                    if ($update_dates['status'] == 2) {

                        //是售后退款
                        TradeEstimates::where(['oid' => $refunds['oid'], 'status' => 0])->update($update_dates);
//                    GoodsSpreadLogs::where(['oid' => $refunds['oid'], 'status' => 2])->update(['status' => 3]);
                    } elseif ($update_dates['status'] == 1) {//取消订单

                        TradeEstimates::where(['tid' => $refunds['tid'], 'status' => 0])->update($update_dates);
//                    GoodsSpreadLogs::where(['tid' => $refunds['tid'], 'status' => 2])->update(['status' => 3]);
                    }
